...
class RFImplantObject:
  ...
  def setupImplantPlacer(self, model):
    ...
    markupsNode.GetDisplayNode().SetGlyphScale(2.0)   # half of default scale
    # center = self.modelCenter(self, model)
    center = self.getIntersect()  # set position implant handle to the intersect point
  ...
  def setupPlacerDisplay(cls, markupsNode):
    ...
    markupsNode.GetDisplayNode().SetOpacity(0)
    showInMainViews(markupsNode)

  @staticmethod
  def modelCenter( model):
      bounds = [0, 0, 0, 0, 0, 0]
      model.GetBounds(bounds)
      return [(bounds[0] + bounds[1]) / 2, (bounds[2] + bounds[3]) / 2, (bounds[4] + bounds[5]) / 2]

  def updateMatrix(self, transform, center, matrix):
  ...
  def applyMatrixToTransform(transform, center):
      def onMatrixModified(matrix, _event=None):
          ...
          # keep implant position at intersect point when slice is rotated
          translation[0, 3] = 0
          translation[1, 3] = 0
          translation[2, 3] = 0
          slicer.util.updateTransformMatrixFromArray(transform, m.dot(translation))

          # We would like MPR line to be not moving with implant
  ...
  def connectMarkupUpdateToModelTransform(self, model, markupsNode, center):
    ...
    # self.updateMatrix(t, center, matrix)
